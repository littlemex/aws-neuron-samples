...
class NeuronGPT2InferenceModel(GPT2PreTrainedModel, GenerationMixin):
    ...
    def store_prefix_emb(self, prefix_emb):
        ...
        The prefill app processes the entire prefix in a single pass, then
        the KV cache is handed off to the decode app. Unlike the 89-step
        sequential decode loop, prefill runs as one batched matmul, so it
        does not accumulate FP16 rounding error.

        Flow:
          1. zero-pad prefix_emb up to max_seq_len
          2. run the prefill app (causal-masked attention) in one shot
          3. copy the prefill KV cache into the decode KV cache
          4. zero out padding positions (prefix_len..max_seq_len-1)
        ...
        # zero-pad prefix_emb up to max_seq_len
        ...
        # last_pos: index of the last valid token
        last_pos = torch.tensor([actual_len - 1], dtype=torch.int32).expand(batch_size)

        # mask: 1 at valid positions, 0 at padding
        mask = torch.zeros(batch_size, max_seq_len, dtype=torch.int32)
        mask[:, :actual_len] = 1

        with torch.no_grad():
            # one-shot prefill: builds the KV cache for positions 0..actual_len-1
            self.neuron_gpt_app.prefill_app(padded, last_pos, mask)

        # copy prefill KV cache into the decode model (padding zeroed)
        self.neuron_gpt_app.sync_kv_cache_prefill_to_decode(actual_len)
        # the decode KV cache now holds correct values at positions 0..actual_len-1
