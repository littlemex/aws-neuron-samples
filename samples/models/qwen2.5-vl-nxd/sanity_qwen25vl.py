#!/usr/bin/env python3
"""Multi-prompt sanity check for the text NEFF.

Runs the Qwen2.5-VL text backbone through NxD Inference (via
HuggingFaceGenerationAdapter) and HF CPU side-by-side, verifying that NxD
produces non-degenerate English / Japanese output. Per-token greedy match
is not required — bf16 numerical drift makes exact agreement impossible at
the layer counts of interest (28 / 64).

Optional env vars:
  WORK_DIR        directory containing hf-ckpt-*l/ and traces/ (default: this script's dir)
  MODEL_ID        HF repo id (default: Qwen/Qwen2.5-VL-7B-Instruct)
  NUM_LAYERS      decoder layer count (default: 28)
  TP_DEGREE       tensor parallel degree (default: 2)
  MAX_CONTEXT_LEN must match the compile-time value of compile_qwen25vl.py (default: 128)
  MAX_NEW_TOKENS  must match the compile-time value (default: 16)
  USE_CHAT_TEMPLATE  apply tokenizer chat template (default: 1)
  REP_PENALTY     repetition penalty (default: 1.05)
"""
import os
import sys
import json
...
import modeling_qwen25vl_text as m_stock
...
print("=" * 60)
print(f"Qwen2.5-VL text sanity: multi-prompt generate (NEFF reuse)")
print("=" * 60)
