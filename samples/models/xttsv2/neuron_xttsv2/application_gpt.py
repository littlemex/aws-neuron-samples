...
class NeuronApplicationXTTSv2GPT:
    ...
    def sync_kv_cache_prefill_to_decode(self, prefix_len: int):
        """Transfer KV cache from the prefill model to the decode model.

        After running prefill, the prefill model's KV cache buffer holds
        the correct context at positions 0..prefix_len-1. Padding
        positions (prefix_len..max_seq_len-1) keep stale bias-derived
        values, which must be zeroed before the transfer.

        traced_model.nxd_model is a TorchScript RecursiveScriptModule, so
        Python helpers like read/write_to_neuron_buffer are unavailable.
        Use the .state attribute exposed by TorchScript instead — it is a
        List[Dict[str, Tensor]] keyed by rank.

        Args:
            prefix_len: number of valid prefix tokens (padding starts here)
        """
        ...
        for rank in range(tp_degree):
            for i in range(n_layer):
                for name in (f"blocks.{i}.attn.cache_k", f"blocks.{i}.attn.cache_v"):
                    ...
                    # Move to CPU before masked multiply: avoids
                    # non-contiguous slice ops on Neuron device tensors.
                    src_cpu = src.cpu()
