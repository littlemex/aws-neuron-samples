...
def compile_transformer_v3_cfg(args):
    ...
    with NxDParallelState(world_size=world_size, tensor_model_parallel_size=tp_degree):
        ...
        # NOTE: load_file() returns mmap-backed tensors; overwriting the same file
        # invalidates the mmap and causes safetensors EFAULT (Bad address, errno 14).
        # We clone tensors and write to a temp file then atomic-rename.
        ...
        for rank in range(tp_degree):  # Only TP checkpoints are created, CFG duplicates them at load time
            ...
            # Load and immediately clone every tensor so we no longer hold mmap references.
            shard_data_raw = load_file(shard_file)
            shard_data = {k: v.clone().contiguous() for k, v in shard_data_raw.items()}
            del shard_data_raw

            original_count = len(shard_data)
            original_size = sum(v.numel() * v.element_size() for v in shard_data.values())

            already_clean = not any('master_weight' in k for k in shard_data.keys())
            has_rank = (not global_rank_state) or all(k in shard_data for k in global_rank_state.keys())
            if already_clean and has_rank:
                print(f"  tp{rank}: already stripped+rank-stamped, skip")
                continue
            ...
            if global_rank_state:
                cleaned.update({k: v.clone().contiguous() for k, v in global_rank_state.items()})
...
if __name__ == "__main__":
    ...
    parser.add_argument("--compiled_models_dir", type=str, default="/mnt/local/compiled_models")
    parser.add_argument("--compiler_workdir", type=str, default="/mnt/local/compiler_workdir")
